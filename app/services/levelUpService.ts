...
export function resolveLevelUpEvents(
  ...
  const subclassDef = subclassId
    ...
    : undefined
  const subclassLevelFeatures = subclassDef?.levels.find(l => l.level === newLevel)?.features ?? []
...
export function isChoiceEvent(event: LevelUpEvent): event is ChoiceLevelUpEvent {
  return ['CHOOSE_SPELL', 'CHOOSE_FEAT', 'ABILITY_SCORE_IMPROVEMENT', 'CHOOSE_SUBCLASS', 'CHOOSE_SKILL'].includes(event.type)
}
  ...
  for (const event of events) {
    switch (event.type) {
      case 'ADD_HP': {
        ...
        const gain = baseHp + event.conBonus

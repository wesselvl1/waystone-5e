...
function resolveEvents() {
  ...
  allEvents.value = resolveLevelUpEvents(character.value, targetClassId.value, newLevel, pack)
...
function confirmASI() {
  if (asiFeatMode.value && selectedFeatId.value) {
    resolvedChoices.value.push({ type: 'RESOLVED_CHOOSE_FEAT', featId: selectedFeatId.value })
  }
  else {
...
const addHpEvent = computed(() => automaticEvents.value.find(e => e.type === 'ADD_HP') as { type: 'ADD_HP'; roll: number; average: number; conBonus: number } | undefined)
        ...
        <template v-if="currentChoice.type === 'ABILITY_SCORE_IMPROVEMENT'">
          ...
          <div class="flex gap-2 mt-2">
            ...
            <button
              ...
              :disabled="asiFeatMode ? !selectedFeatId : asiPointsUsed < asiPoints"
              @click="confirmASI"
            >Confirm</button>
          </div>
        </template>
        ...
        <template v-else-if="currentChoice.type === 'CHOOSE_FEAT'">
          ...
          <div class="space-y-2 max-h-72 overflow-y-auto">
            ...
            >
              ...
              <p v-if="feat.prerequisite" class="text-xs text-slate-500 mt-0.5">Prerequisite: {{ feat.prerequisite }}</p>
            </button>
          </div>
          <div class="flex gap-2 mt-2">
            <button class="btn-ghost flex-1 text-sm" @click="skipChoice">Skip</button>
            <button class="btn-primary flex-1 text-sm" :disabled="!selectedFeatId" @click="confirmFeat">Confirm</button>
          </div>
        </template>
      ...
      <template v-if="wizardStep === 'summary'">
        ...
        <div v-if="addHpEvent" class="card space-y-3">
          ...
          <div class="grid grid-cols-2 gap-2">
            ...
            >
              <p class="text-2xl font-bold text-white">+{{ Math.max(1, addHpEvent.average + addHpEvent.conBonus) }}</p>
            ...
            <button
              ...
              @click="hpChoice = 'roll'"
            >
              <p class="text-2xl font-bold text-white">+{{ Math.max(1, addHpEvent.roll + addHpEvent.conBonus) }}</p>
            ...
            <button
              ...
              @click="hpChoice = 'max'"
            >
              <p class="text-2xl font-bold text-white">+{{ Math.max(1, addHpEvent.max + addHpEvent.conBonus) }}</p>
            ...
            >
              <p class="text-2xl font-bold text-white">+{{ hpChoice === 'manual' ? Math.max(1, manualHp + addHpEvent.conBonus) : '?' }}</p>

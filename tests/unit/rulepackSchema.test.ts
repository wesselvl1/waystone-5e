...
describe('RulepackSchema', () => {
  ...
  it('rejects a rulepack with a missing required field', () => {
    const { classes: _, ...noClasses } = minimalRulepack
    const result = RulepackSchema.safeParse(noClasses)
    expect(result.success).toBe(false)
  })

  it('rejects a race with negative speed', () => {
    const result = RulepackSchema.safeParse({
      ...minimalRulepack,
      races: [{
        id: 'elf', name: 'Elf', size: 'medium', speed: -5,
  ...
  it('rejects a race with an invalid size', () => {
    const result = RulepackSchema.safeParse({
      ...minimalRulepack,
      races: [{
        id: 'dwarf', name: 'Dwarf', size: 'colossal', speed: 25,
        abilityScoreBonuses: {}, traits: [], languages: [],
      }],
    })
    expect(result.success).toBe(false)
